...
def test_attributes_round_trip(synthetic_ply, tmp_path):
    ...
    # The rows that survive bbox [-1,1]^3 are indices 0..7 and 12..15
    # (alpha-low + oversized + sphere — none had x>1).
    survived = []
    for i in range(n):
...
def test_dbscan_keep_largest(synthetic_ply, tmp_path):
    ...
    near origin, not the (3,3,3) cluster (which is also tight). The
    'largest' here is the origin-area cluster — count of dense points
    near origin > count near (3,3,3). 12 points sit within a tight
    radius near origin (idx 0..7, 12..15) and 4 sit near (3,3,3)."""
    ...
    after = _read_attrs(Path(res["ply"]))
    # Largest cluster sits near origin — none of the kept points should
    # be at the (3,3,3) cluster or the floaters at +/-10.
