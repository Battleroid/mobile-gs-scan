 ...
 *
 * Three depth/distance cues stack in the rendering pipeline so
 * the eye reads the overlay as 3D rather than as a flat sprinkle
 * of dots over the camera image:
 *
 *   1. Distance-attenuated point size in the vertex shader:
 *      `gl_PointSize = clamp(BASE / max(w, 0.1), MIN, MAX)`.
 ...
 *      and softens the edge with a smoothstep. GL_POINTS draws
 *      as square sprites by default; the discard-and-feather
 *      makes them look like crisp circles.
 *
 * Lifecycle:
