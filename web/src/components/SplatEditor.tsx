...
export function SplatEditor({ scene }: Props) {
  ...
  async function onApply() {
    ...
    try {
      ...
      // The websocket will flip edit_status. Stay on preview until the
      // edited artifact is ready.
    } catch (e) {
...
function PreviewSplatScene({
  ...
  const applyMask = useCallback(
    (state: RecipeState) => {
      ...
      if (
        ...
        // Spark API: setSplat(index, position, scales, quat, color, opacity).
        // We toggle alpha to zero for hidden, original for visible. Spark
        // internalises the state per-gaussian — no buffer re-upload needed.
        for (let i = 0; i < count; i++) {
          try {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Always parse the .ply ourselves so we have xyz + opacity for the
        // mask. Spark renders from its own URL fetch; on Spark-less hosts
        // we render the parsed point cloud as a fallback.
        const { PLYLoader } = await import(
          "three/examples/jsm/loaders/PLYLoader.js"
        );
        const loader = new PLYLoader();
        ...
        if (mod && !cancelled) {
          ...
          // Fallback: render the parsed point cloud and modulate point
          // size to "hide" rejected points. setSplat-equivalent on a
          // PointsMaterial isn't available, so we use a per-vertex size
          // attribute via a custom shader chunk.
  ...
  // Re-apply mask whenever the recipe changes (debounced via effect tick).
  useEffect(() => {
